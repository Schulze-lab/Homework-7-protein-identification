# Assignments Week 6
# Introduction
The goal of this assignment is to perform a protein database search for one of the publicly available datasets for Methanococcus maripaludis. This search should identify a range of peptides and their corresponding proteins. In the lab, we will go through the usage of the Python framework Ursgal, but you are welcome to use any protein database search that you see fit.
# Input data
You can use the .mzML files from the PXD047095 dataset that have been transferred to the Oedipus server already, or you can download and convert any other M. maripaludis dataset, e.g. one that is useful for your group project. In addition, you will need a .fasta file for the corresponding reference proteome (downloaded from UniProt).
...
4)	Count the number of identified peptides and their corresponding proteins based on the generated result file(s). Make sure to count unique peptides and proteins (i.e. don’t count the same peptide or protein multiple times).
...
8)	Commit your scripts, plots, and final result files to your GitHub Classroom repository. Do NOT commit large input files (like .raw, .mzML, etc)!
...
# Submission
You must submit the assignment through GitHub Classroom by 8 am ET on Feb 26 to get full credit. 
